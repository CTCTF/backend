			...
			common.db.collection("submissions").find({
				...
				common.db.collection("problems").find({
					...
					if (err3) {
						...
						for(var i=0; i<Math.min(5, teams.length); i++) {
							teamJson.push({
								tid: teams[i].tid,
								place: (i+1),
						...
						async.each(teamJson, function(team, callback) {
							...
							var finalData = {};
							finalData.options = {
								title: "CTCTF 2014 Score Progression",
								/* curveType: "function", */
								height: 348,
								legend: {
									"position": "bottom"
						...
						for(var i=0; i<teams.length; i++) {
							...
							for(var j=0; j<submissions.length; j++) {
								if (submissions[j].tid.indexOf(teams[i]._id.valueOf()) > -1) {
									...
									var prob_pid;
									for(var k=0; k<problems.length; k++) {
										if (submissions[j].key.toLowerCase().indexOf(problems[k].flag.toLowerCase()) > -1) {
											if (probs_counted.indexOf(problems[k]._id.valueOf()) == -1) {
												// console.log(problems[k].basescore);
												probs_counted.push(problems[k]._id.valueOf());
												points += problems[k].basescore;
											}
											break;
										}
									}
