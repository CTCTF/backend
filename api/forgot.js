		...
		common.db.collection("reset_password").update({
			...
			common.db.collection("reset_password").insert({
				...
				sendgrid.send({
					...
					from: "team@ctctf.io",
					fromname: "CTCTF Team",
					replyto: "team@ctctf.io",
					subject: "Password Reset Requested.",
					html: '<p>Hey there</p><p>Someone (hopefully you) requested to change the password for the account with the email ' + req.param('email') + '. Obviously, we\'ll be asking for verification of identity, so if you did in fact request this, follow this link: http://ctctf.io/forgot/' + code + "</p><p>CTCTF Team</p>"
				}, function(err4, json) {
					if (err4) {
						res.send({
							success: 0,
