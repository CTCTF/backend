							...
							*/
							if (true) { // b) {
								unlocked.push({
									...
									correct: correctPIDs.indexOf(p.flag) != -1,
		...
		common.db.collection("submissions").find({
			...
			for(var i=0; i<submissions.length; i++) {
				for(var j=0; j<problems.length; j++) {
					if (submissions[i].key.toLowerCase().indexOf(problems[j].flag.toLowerCase()) > -1 && solved_names.indexOf(problems[j].grader) == -1) {
						solved_names.push(problems[j].grader);
						solved.push({
